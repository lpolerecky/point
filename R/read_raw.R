...
#' counts.
#' \code{read_meta()} can be used to solely retrieve the metadate associated
#' with ion count data analysis; loading specifications related to the optics,
#' the primary and secondary ion beam, and mass spectrometer settings.
#'
  ...
  f.PHD <- function(a, b, c) {
         ...
         readr::read_table2(paste0(directory, "/", a),
                            ...
                            col_names = c("num.mt",
                                          ...
                                          "EMHV"),
                            col_types = "-cddd",
                            na = c(mapply(strrep,"X", 1:10, USE.NAMES =  FALSE),
                                   "1.#R"),
                            skip = b[1] - 1,
                            n_max = c)
  }
...
# Length check of txt files
  if (any(purrr::map_dbl(l.c, ~length(read_lines(paste0(directory, "/", .),
                                                 n_max = 2))) == 0)) {

    good <- purrr::map_dbl(l.c, ~length(read_lines(paste0(directory, "/", .),
                                                   n_max = 2))) > 0
...
# Column content check of txt files
  if (any(purrr::map_dbl(l.c, ~nrow(read_tsv(paste0(directory, "/", .),
                                             comment = "B",
                                             skip = 1,
                                             col_names = c("t", "N"),
                                             col_types = "-cc",
