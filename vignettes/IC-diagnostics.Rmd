...
Precision monitoring by combined predictive and descriptive statistical analysis informs on errors that reflect on a combination of the random nature of secondary ion generation, purely analytical biases and artifacts resulting from the homogeneity of the analytical substrate. In an "ideal" isotopically homogeneous analytical substrate the count rates of two isotope from the same element should be dependent on each other. Thus, even though the ionization efficiency might vary within a single analysis, the count rate of an isotope species $b$ can be deduced from the count rate of isotope species $a$ through a linear combination with the isotope ratio $R$:
...
where, a vector of independent values $x$ and a vector of coefficients ($\beta$) can be used to express the conditional mean of $y$ at any $i$
...
```{r regression, echo=FALSE,  fig.cap = "Regression",  fig.width=8, fig.height=6}
...
# simulate isotope ion counts
sim_R <- purrr::map_dfr(var_R, ~sim_R(tb.R, M_R_Xt.pr, ion1 = "13C", ion2 = "12C", sys = 0, type = .x))
sim_R_sys <- purrr::map_dfr(var_R, ~sim_R(tb.R, M_R_Xt.pr, ion1 = "13C", ion2 = "12C", sys = 0.1, type = .x))

# diagnostics and plots of simulations
gg.sim <- plot_RDiag(sim_R, Xt.pois, N.pois, species, "13C", "12C", file.nm)
gg.sim_sys <- plot_RDiag(sim_R_sys, Xt.sys, N.sys, species, "13C", "12C", file.nm)

library(patchwork)
gg.sim[[1]] / gg.sim_sys[[1]]
```




```{r gradient, echo=FALSE,  fig.cap = "Regression",  fig.width=8, fig.height=6}

sim.R <- purrr::map_dfr(var_R, ~sim_R(tb.R, M_R_Xt.pr, reps = 100, ion1 = "13C", ion2 = "12C", sys = 0.1, type = .x))

# Diagnostics
sim.dia <- diag_R(sim.R,
                     method = "CooksD",
                     args = expr_R(Xt = "Xt.pois",
                                   N = "N.pois",
                                   species = "species",
                                   ion1 = "13C",
                                   ion2 = "12C"),
...
# Original
sim.R <- stat_R(sim.R, Xt.pois, N.pois, species, ion1 = "13C",
                ion2 = "12C", file.nm) 

# Augmentation
sim.aug <- sim.dia %>%
  filter(flag_CD == "non-influential") %>%
  stat_R(., Xt.pois, N.pois, species, ion1 = "13C",
         ion2 = "12C", file.nm)

sim.trj <- left_join(sim.R, sim.aug, by ="file.nm", suffix = c(".org", ".aug")) %>% 
  mutate(theta = log(RSeM_R_Xt.pois.org / RSeM_R_Xt.pois.aug),
         hat_theta = log(hat_RSeM_R_Xt.pois.org / hat_RSeM_R_Xt.pois.aug)) %>%
  tidyr::separate(file.nm, sep = "-", c("file.nm", "rep", "exp"))
...
ggplot(sim.trj , aes(x = theta, y = hat_theta)) +
  stat_density_2d(geom = "polygon", aes(alpha = stat(level), fill = file.nm), color = "grey") +
  geom_point(shape = 3) +
...
```{r normres, echo=FALSE, fig.cap = "Quantile-Quantile plot to test normality of residuals",  fig.width=8, fig.height=6}
gg.sim[[5]] / gg.sim_sys[[5]]
