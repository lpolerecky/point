...
gg.sim.ex <- plot_RDiag(sim_IC_extremes, Xt.sim, N.sim, species, "13C", "12C", simulation, trend)
...
gg.sim <- plot_RDiag(sim_IC_ideal, Xt.sim, N.sim, species, "13C", "12C", simulation, trend)
...
Effect size can be unified in a single measure, the explained variance $\eta^2$, obtained from the sum of squared deviations from the $\bar{R}$ explained by the augmentation ($SSA$) divided by the $SST$ of $R$. 

\begin{equation} 
  \eta^2 = \frac{SSA}{SST} = \frac{n'\hat{S}_R^2}{\sum_{i=1}^n (R - \bar{R})^2}
\end{equation} 

This variance function can be converted to a $\chi^2$ goodness of fit test with 1 degree of freedom. The $H0$ is that the augmentation brings about no change in the variance of the ion count data. In other words if the $H0$ is not rejected there is no reason to assume that the data would originate from a substrate with a heterogenous isotopic composition.  

  
```{r echo=FALSE, fig.width=8, fig.height=4}

CD_ini <- filter(CooksD_stepwise, execution == 2)
...
```{r ion, fig.width=8, fig.height=6, fig.cap="The blockwise mean plotted against consecutive blocks. Note that the none-isotope ratios show a decreasing trend over time representing differential change in the ionization potential.", echo=FALSE}
...
Structural trends of a variable (geochemical proxies) based on the independent variable time. Two families of approaches can roughly be divided *global* and *local* regression.  The simplest case is the traditional linear regression model:

$$ y_t = \beta_0 + \beta_1x_t + \epsilon_t$$

The General Additive Model (GAM) is an extension of this model with addition of a smoothing function replacing the linear term of the previous function:

$$ y_t = \beta_0 + f(x_1) + \epsilon_t$$

I have used the package `mgcv`, which has the function `s()`, to define the smooth in the function `gam`. For these first set of global models I extract the goodness of fit as it explain how well the specific model type can explain the variance in the timeseries
...
tb.mod <-ion(df = tb.zero, args = expr_ion(Xt = "Xt.pr", N = "N.pr", t = "t.rw", species = "species.nm"), file.nm) 


tb.R.mod <- stat_R(tb.mod, 
                                  Xt.pr.detrended,
                                  N.pr.detrended,
                                  species.nm, 
                                  "12C", 
                       ...
                       filter(file.nm == "2018-01-19-GLENDON_1_2")
                           


ggplot(tb.R.mod, aes(x =bl.mt, y = M_R_Xt.pr.detrended)) +
  geom_point() +
  facet_wrap(vars(R.nm), scales = "free")
