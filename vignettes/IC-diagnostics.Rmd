...
For a systematic evaluation of the results of the regression diagnostics, a measure of the model fit is required. A goodness of fit 





```{r gradient, echo=FALSE,  fig.cap = "Regression",fig.width=6, fig.height=4}


# Plot goodness of fit performance
ggplot(CooksD_results, aes(x = execution, y = hat_Chi2, color = simulation)) +
  geom_point(alpha = 0.1 ) +
  facet_grid(rows = vars(trend))
...
A comparison of methods can be accomplished by measuring the effect size of the method. That is how much of the variance is explained by the augmentation, where the sum of squared errors removed by augmentation (SSA) and the sum of squared of the original dataset (SST).
 
\begin{equation} 
  \omega^2 = SSA / SST = \frac{\sum_{i=1}^n (R - \bar{R})^2 - \sum_{i=1}^n* (R* - \bar{R}*)^2}{\sum_{i=1}^n (R - \bar{R})^2}
\end{equation} 


```{r echo=FALSE, fig.width=8, fig.height=8}

tb.trj <- CooksD_stepwise  %>% 
...
ggplot(tb.trj , aes(x = var_R, y = diff_n, color = simulation)) +
  ...
  facet_grid(cols = vars(trend), rows = vars(execution)) +
  ggtitle("simulations")
