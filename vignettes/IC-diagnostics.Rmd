...
The output consist of a list with two `tibbles` for each element; the `"df"` tibble contains the dataset belonging to the `"results"` tibble, and the list length is determined by the `reps` argument (reps + 1). The specific diagnostics included in the `"results"` tibble depend on the chosen method, and are delineated below.
...
gg.sim.ex <- plot_diag_R(ls.sim, args = expr_R_stat, simulation, trend)
...
gg.sim <- plot_diag_R(ls.sim2, args = expr_R_stat, simulation, trend)
...
ggplot(tb.trj , aes(x = diff_RS, y = diff_n, color = simulation)) +
  geom_point(alpha = 0.3) +
  scale_alpha(guide = "none") + 
  facet_grid(cols = vars(trend), 
             rows = vars(execution), 
             labeller = labeller(execution = label_parsed)
             )+
  ggtitle("Cook's D") +
  ylab("n'") +
...
tb.trj <- CM_eval %>% 
  filter(execution == 2 | execution == 3 | execution == 7) %>% 
  mutate(execution =  factor(execution, 
                             labels = c(bquote(1^"st"~"iteration"),
                                        bquote(2^"nd"~"iteration"),
