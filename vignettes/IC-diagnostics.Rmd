...
ls_aug <- purrr::map2(
  ion1, 
  ion2, 
  ~diag_R(tb_pr, .x, .y, file.nm, bl.nm, .method = "Cameca", .plot = F, .reps = 2)
  )       


```

If the argument`plot = FALSE`, the output consist of a list with two `tibbles` for each element; the `"df"` tibble contains the dataset belonging to the `"results"` tibble, and the list length is determined by the `reps` argument (reps + 1). The specific diagnostics included in the `"results"` tibble depend on the chosen method, and are delineated below. Setting the argument `plot = TRUE` results in a [ggplot](https://cran.r-project.org/web/packages/ggplot2/vignettes/ggplot2.html)  (if `plot_type = "static`) or when `plot = TRUE` in a dynamic [plotly](https://plotly.com/ggplot2/getting-started/)  (if `plot_type = "interactive`) version of the ggplot, alongside the list of tibbles.
...
# ratios (blockwise)
 ...
 fun_CAMECA <- function(df){
  
  filter(df, file.nm  == "2018-01-19-GLENDON_1_1", execution == 2) %>% 
...
# Call the function
# tb.CAMECA <- 
  purrr::map_dfr(ls_aug, fun_CAMECA) %>% 
    group_by(`Ratio#`) %>% 
    arrange(`Block#`)
...
fun_CAMECA_gl <-function(df, a, b){
  stat_R(df, Xt.pr, N.pr, species.nm, a, b, 
         file.nm, output = "complete"
         ) %>%   
    filter(file.nm == "2018-01-19-GLENDON_1_1") %>%
    mutate(`Err_mean (%)` = RSeM_R_Xt.pr / 10,
           `Poisson (%)` = hat_RSeM_R_Xt.pr / 10,
           `Ratio#` = paste(a, b, sep = "/"),
           SD_bl = sd(R_Xt.pr) / mean(R_Xt.pr) * 100
           ) %>% 
...
1) The **"ideal"** substrate with a completely homogeneous composition  
...
In the two simulations with a heterogeneous isotope composition the total span is set to 60‰. The same simulations have been repeated with a linear trend superimposed on the count rates of both isotope species, which is an often encountered phenomenon and relates to changes in the ionization potential, and, as discussed before (see vignette: [IC-process](IC-process.html)), this effect afflicts both isotope species to a similar degree thereby leaving the $R$ unaltered [see @Fitzsimons2000a]. This ionization effect does, however, effect regression diagnostics. 


```{r simulations, echo=FALSE, fig.width=12, fig.height=6, fig.cap="Histograms comparing input and output $R$ values for simulations of an ideal isotopically homogenous material, a gradual gradient in $R$ (symmetric gradient), and a sudden shift in $R$ (asymmetric gradient). Note, that the linear ionization trend is measured as the relative deviation of the major ion as parts per mille."}
...
From Figure \@ref(fig:simulations) is is directly evident that the real variation in $R$ (input R; red) is indistinguishable in the $R$ derived from the ion counts (output R; blue). This is an effect related to the relative large random variation induced by the ionization of the substrate. So, even for these large variations in isotopic composition, it cannot be confidently said whether the measured $R$ is representative for the analysed site, or just some averaged value of an otherwise isotopically variable substrate. This effect is, furthermore, hardly observable in the precision of the data, where the $\epsilon_R$ of the simulation dubbed *asymmetric* is `r tb.R %>% filter(simulation == "asymmetric", trend == "linear trend (var: 0)") %>% pull(RSeM_R_Xt.sim) %>% unique() %>% round(2)`‰, whereas the *ideal* simulation has only a marginally better precision of `r tb.R %>% filter(simulation == "ideal", trend == "linear trend (var: 0)") %>% pull(RSeM_R_Xt.sim) %>% unique() %>% round(2)`‰. This univariate approach will, however, allow distinguishing these analyses based on the mean (*asymmetric* simulation: `r tb.R %>% filter(simulation == "asymmetric", trend == "linear trend (var: 0)") %>% pull(M_R_Xt.sim) %>% unique() %>% calib_R(standard = "VPDB", type = "composition", input = "R", output = "delta") %>% round(2)`‰ and *ideal* simulation: `r tb.R %>% filter(simulation == "ideal", trend == "linear trend (var: 0)") %>% pull(M_R_Xt.sim) %>% unique() %>% calib_R(standard = "VPDB", type = "composition", input = "R", output = "delta") %>% round(2)`‰), even though, an average values is not really representative for the former case as the original population of $R$ is highly variable and skewed. So, clearly, a different approach must be adopted to identify these isotopically highly variable substrates.


<!--  From the previous examples it becomes apparent that even largely heterogeneous substrates cause only minimal deviation in the internal precision. This could be interpreted that their is perhaps a lot of hidden variation in the nominal SIMS isotope analysis, which skews $\bar{R}$ but leaves precision untouched. -->
...
\begin{equation}
  E(y_i|x_i) = \beta_0 + \beta_1 x_i + e_i \;\;\; \text{where} \;\;\; e_i \sim N(0,\sigma^2) \;\;\; \text{with} \;\;\; E(e_i|x_i) = 0
    (\#eq:conmean)
\end{equation}
...
\begin{equation}
 \hat{X}_i^b = \bar{R}X_i^a +  e_i \;\;\; \text{where} \;\;\; e_i \sim N(0,\sigma^2) ;\;\; \text{with} \;\;\; E(e_i|x_i) = 0 \;\;\; \text{if} \;\;\; X_i^a, X_i^b \gg 0
  (\#eq:linmodR)
\end{equation}
...
\begin{equation}
 \hat{e}_i =  X_i^b - \bar{R}X_i^a  
  (\#eq:resR)
\end{equation}


```{r regression, echo=FALSE,  fig.cap = "Cross plotted ^13^C and ^12^C count rates and the regression line following equation, based on the linear function of Eq. \\@ref(eq:linmodR) and with 2 times the standard error of the regression represented as the green area. Here the red dots are defined as outliers since they are outside 2 times the standard error of the regression. This is essentially the same procedure as performed by the Cameca software.", fig.width=12, fig.height=6}

# expression of stat_R()
...
plot_Rm <- diag_R(sim_IC_extremes, 
                  method = "Rm", 
                  args = expr_R_stat,
                  reps = 1,
                  simulation,
...
The error estimate $\hat{e}_i$ can be used to validate three important assumptions of the previously adopted model (Eq. \@ref(eq:linmodR)):

1) $\hat{e}_i$ should vary independently from the predicted value (e.g. constant variance)
2) $\hat{e}_i$ should be approximately normally distributed with a mean of zero (e.g. normality of residuals)
3) $\hat{e}_i$ should be independent random variables over time (e.g. independence of residuals)

To over come scale-dependence of the residuals, the residuals will first be normalized. This can be done by using the *studentized* residuals ($\hat{e}_{i}^*$). A studentized residual is obtained by dividing the residual by an independent estimate of the standard deviation of the  residuals. To ensure independence, the standard deviation of the residuals is calculated by leaving the residual of the $i$-th observation out:

\begin{equation}
  \hat{e}_i^* = \frac{\hat{e}_i}{S_{\hat{e}_{(-i)}} \sqrt(1-h_i)}
  (\#eq:studres)
\end{equation}
...
Given that the "ideal" simulation satisfies the conditions as initially described for the linear model Eq. \@ref(eq:linmodR), the estimated $\hat{e}}$ can be used to assess deviations from this "ideal linear model", and thereby provide a bivariate measure of how representative the $\bar{R}$ is for the sampled spot. The goal of residual analysis would therefore be to identify the influence of the coefficient; in this case the influence on $\bar{R}$, by determining the leverage and discrepance (outlyingness) of variable $X^a$, such that:
...
plot_normE <- diag_R(sim_IC_ideal, 
                     method = "norm_E",
                     args = expr_R_stat,
                     reps = 1,
                     simulation,
...
The effectiveness of recognising and differentiating the "ideal" substrate from compromised samples with Cook's D; the so-called "effect size", can now be calculated by assessing the variability reduction in the augmentation of the original dataset. This can be achieved by combining two measures which are sensitive to the changes brought about by the *Cooks D* augmentation. The first being the change in $n' = n - n^*$ required to reduce the variance of the ion count data, and, secondly, the variance of $R_i$ around the mean $\bar{R}$ explained by the augmentation. This can be gauged with the $\epsilon_R^2$, as it is a measure of variance independent from changes in the mean. 

\begin{equation}
   \epsilon_{R}^{2\,'} = \epsilon_{R}^{2}   - \epsilon_{R}^{2\,*}
   (\#eq:varExp)
\end{equation}  

  
This has been illustrated in Figure \@ref(fig:effectsize), which show the change along these two measures over subsequent iterations ($z = 1, 2, ..., Z$).

```{r effectsize, echo=FALSE, fig.width=8, fig.height=6, fig.cap="The trjactories of the simulated ion coun data over multiple augmentation cycles according to the difference in $n$ and the explained variance Eq. \\@ref(eq:varExp)."}

tb.trj <- CD_eval 


# %>% 
#   filter(execution == 2 | execution == 3 | execution == 6) %>% 
#   mutate(execution =  factor(execution, 
#                              labels = c(bquote(1^"st"~"iteration"),
#                                         bquote(2^"nd"~"iteration"),
#                                         bquote(7^"th"~"iteration")
#                                         )
#                              )
#         )

gg <- ggplot(CD_eval, aes(x = diff_RS, y = diff_n, color = simulation, frame = execution)) +
  geom_point(alpha = 0.3) +
  scale_alpha(guide = "none") + 
  facet_grid(cols = vars(trend))+
  ggtitle("Cook's D") +
  ylab("n'") +
  xlab(expression(epsilon[R]^2*""^"`"~"\u2030"))

plotly::ggplotly(gg)

```

This effect size (Fig. \@ref(fig:effectsize)) can be unified in a single measure, the explained variance $\eta^2$, where the sum of squared relative deviations from the $\bar{R}$, or RST; constitutes:

\begin{equation}
  RST = \epsilon_{R}^{2} \times (n-1) = \sum_{i = 1}^{n} (\frac{x_{i}}{\bar{x}} - 1)^{2}  
  (\#eq:RST)
\end{equation}

$$ RST = \epsilon_{R}^{2} \times (n-1) = \sum_{i = 1}^{n} (\frac{x_{i}}{\bar{x}} - 1)^{2} $$

where the augmentation effectively splits the variability in the dataset, as follows:

\begin{equation}
 RST = RSA + RSE = \epsilon_{R}^{2*} \times n^{*} + \epsilon_{R}^{2`} \times n^{`}
 (\#eq:RSA)
\end{equation}

 $$RST = RSA + RSE = \epsilon_{R}^{2*} \times n^{*} + \epsilon_{R}^{2`} \times n^{`}$$
<!-- obtained from the  explained by the augmentated model ($SSA$) divided by the $SST$ of $R$. Here $SSE$ is the variance by the following hypothetical  model as follows -->

The division of variability among RSA ad RSE by the *Cooks D* augmentation yields an effect size measure: 

\begin{equation} 
  \eta^2 = \frac{RSE}{RSA} = \frac{RST}{RSA} - 1 =  \frac{\epsilon_{R}^{2'} \times n^{'}}{\epsilon_{R}^{2*} \times n^{*}}
  (\#eq:eta)
\end{equation} 

$$ \eta^2 = \frac{RSE}{RSA} = \frac{RST}{RSA} - 1 =  \frac{\epsilon_{R}^{2'} \times n^{'}}{\epsilon_{R}^{2*} \times n^{*}} $$

This yields a variance function which can be converted to a $\chi^2$ goodness of fit test, where the coefficient $\bar{R}'$ can divided as a hypothesis with $H0 : \bar{R}' = 0$ and $Ha : \bar{R}' \ne 0$. This test has 1 degree of freedom, as there are 2 relations ($r$) used to calculate the coefficient estimate ($\bar{R}$ from species $a$ and $b$) of the linear model ($df = r - 1$ for goodness of fit).  In other words, the $H0$ suggest that the augmentation brings about no change in the variability of the ion count data, so that if the $H0$ is rejected there is reason to assume that the data originates from a substrate with a heterogeneous isotopic composition.  

  
```{r mixmodel, echo=FALSE, fig.cap="The $\\chi^2$ values for the Cook's D augmentation, based on the variance function Eq. \\@ref(eq:eta) for all three simulations and for two types of linear trends in the ionization efficiency. Note that the dotted lines represents the $\\chi^2$ distribution with 1 df and 95% confidence interval, and the dashed line represents a mixture distribution combining the $\\chi^2$ distribution with the null (\"ideal\") distribution based on Monte Carlo simulations.", fig.width=8, fig.height=4}
...
fx <- function(x, var = 0) { 
  ...
  dist <- filter(null_dist, trend == paste0("linear trend (var: ", var, ")"))
...
fx_dist <-lst(
              a <- df_den %>% 
                mutate(probs = fx(vec_den, var = 0),
                       trend = paste0("linear trend (var: ", 0, ")")
                       ),
              b <- df_den %>% 
                mutate(probs = fx(vec_den, var = 500),
                       trend = paste0("linear trend (var: ", 500, ")")
                       )
              ) %>% 
  reduce(bind_rows)
      
ggplot(df_den, aes(x = vec_den)) +
  geom_line(data = fx_dist, aes(x = vec_den, y = probs), linetype = 2) +
  ...
  geom_point(data = tb.xrp, 
             aes(x = chi_n2 , 
                 y = dchisq(chi_n2 , df = df),
                 color = simulation, 
                 alpha = 0.2
                 ),
             position = jitter
  ...
  annotate("text", label = "95% confidence limit \n mixture distribution", 
           x = y_star * 1.1, y = 0.5, angle = 90, size = 3
           ) +
  annotate("text", label = "95% confidence limit \n Chi distribution", 
           x = upper95 * 1.1, y = 0.5, angle = 90, size = 3, 
           color = "lightgrey"
           ) +
...
Based on the simulated data in Figure \@ref(fig:mixmodel) it is evident that the "ideal" model can clearly be differentiated from symmetric and asymmetric isotope variability, although the test is less sensitive to symmetric isotope variation with an increasing linear trend caused by the ionization efficiency. The Monte Carlo simulation of the "ideal" data-set can be used to increase the sensitivity of the model. This yields a mixture model with two additive components; an $\chi^2$ distribution with 1 df and a null distribution (based on the "ideal" data set). The $\eta^2$ metric for isotope ion counts can thus be used to assess the probability of a measured spot representing an isotopic composite, or a truly homogeneous isotopic substrate. 


```{r flags, echo=FALSE, eval=FALSE}

 purrr::map(c("CV", "QQ"), ~diag_R(sim_IC_extremes, 
                            method = .x,
                            args = expr_R_stat,
                            reps = 1,
                            simulation, 
...
Conveniently the same effect size measure can be applied to the univariate method (the default block-wise procedure of the Cameca software), so that a comparison of methods is attainable. Figure \@ref(fig:effectsize2) show the $\epsilon_{R}^{2'}$ and the $n'$ explained by the augmentation, based on the Cameca procedure. 

```{r effectsize2, echo=FALSE, fig.width=8, fig.height=6, fig.cap="The trajectories of the simulated ion count data over multiple augmentation cycles according to the difference in $n$ and the explained variance by the univariate Cameca method."}

gg <- ggplot(CM_eval, aes(x = diff_RS, y = diff_n, color = simulation, frame = execution)) +
  geom_point(alpha = 0.3) +
  scale_alpha(guide = "none") + 
  facet_grid(cols = vars(trend))+
  ggtitle("Cameca") +
...
Converting the $\eta^2$ of this method to $\chi^2$ (as before)(Fig \@ref(fig:CamChi)) shows that this univariate approach is not able to identify the isotopic variable simulations (Fig. \@ref(fig:CamChi)). Moreover, in the case of a linear trend induced by ionization variance yields an differentiation opposite of the desired effect. Thereby once more showing the problems associated with this method.  

```{r CamChi, echo=FALSE, fig.cap="The $\\chi^2$ values for the Cameca-style augmentation, based on the variance function Eq. \\@ref(eq:eta) for all three simulations and for two types of linear trends in the ionization efficiency.", fig.width=8, fig.height=4}

tr.xrp2 <- filter(CM_eval, execution == 2)
...
The univariate method does not allow separation of the simulated data. To conclude, the bivariate *regression diagnostics* is the more sensitive test and can flag problematic analyses, even if this is not obvious from the descriptive and predictive statistics alone.  
