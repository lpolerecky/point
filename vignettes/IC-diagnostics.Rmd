...
ls_dia <- purrr::map2(
  ion1, 
  ion2, 
  ~diag_R(tb_pr, .x, .y, file.nm, bl.nm, .method = "Cameca", .reps = 2, .output = "diagnostic") 
  )       
```
...
These calculations are internally performed by a consecutive call to base `lm()` and `broom::augment()`.
...
Coefficients for this mixed linear model are obtained by Restricted Maximum Likelihood (REML) optimization using the `lme()` function of the `nlme` package. The ratio of the obtained log-likelihood (i.e., maximized likelihood functions) for the restricted model and the unrestricted model, or likelihood-ratio test (LR), can the be applied to deduce the significance of including a random component in Equation \@ref(eq:linmodR). Here, a larger difference in the AIC ($\Delta AIC_{\bar{R}}$) and thus lower $p_{\bar{R}}$ indicates that the null model of no inter-isotope variability becomes less appropriate.
