...
Precision monitoring by combined predictive and descriptive statistical analysis informs on errors that reflect on a combination of the random nature of secondary ion generation, purely analytical biases and artifacts resulting from the homogeneity of the analytical substrate. In an "ideal" isotopically homogeneous analytical substrate the count rates of two isotope from the same element should be dependent on each other. 
...
In this ideal relationship the $R$ can be used to calculate the mean count rate of species $a$ at any $i$. And, variation in the count rate is only caused by the random nature of secondary ion generation. which follows a Poisson distribtuion. Thus it is assumed  that the count rate of a species $a$ and $b$ in one count block can be approximated a static conditional mean model .
...
This formulatiuon is also known as the population linear regression function. Instead of using of Ordinary Least Squares (OLS) to obtain estimates for the regression coefficients, the $\beta_1$ can be approximated with $\bar{R}$ based on Eq. \@ref(eq:conR). Furthermore, Thus can be used as an approximation of the slope ($\hat{\beta}_1$) of the regression line. It thus follows that $\hat{\beta}_0$ should approximate $0$, as:
...
sim_ideal <- sim_R(tb.R, M_R_Xt.pr, n = 3000, N_range = 10 ^ 6, reps = 1, sys = 0.5, type = "ideal") 
sim_gradient <-sim_R(tb.R, M_R_Xt.pr, n = 3000, N_range = 10 ^ 6, reps = 1, sys = 0.5, type = "gradient")


gg.sim.ideal <- plot_RDiag(sim_ideal, Xt, N, species, "13C", "12C", file.nm)
gg.sim.gradient <- plot_RDiag(sim_gradient, Xt, N, species, "13C", "12C", file.nm)

library(patchwork)
gg.sim.ideal[[1]] + gg.sim.gradient[[1]]
...
If the $\bar{R}$ of an analysis is derived from an homogenous subtrate and $N_i$ is sufficiently larger for both species, than $\epsilon_i$ should satisfy the following criterion:
