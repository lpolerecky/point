...
theme_replace(axis.title = element_text(size = 9),
...
a <- ggplotGrob(
  ...
  ggplot(tb.zero[[1]], aes(x = n, y = Xt.pr)) +
...
b <- ggplotGrob(
  ...
  ggplot(tb.zero[[2]], aes(x = n, y = Xt.pr)) +
      geom_point(alpha = 0.1) +
      facet_wrap(vars(species.nm), scales = "free") +
      ylab("X (ct/sec)") +
      xlab("t")
...
```{r ionR3, echo=FALSE, fig.width=8, fig.height=4, fig.cap="GAM predicted model fit (white line) to single ion count rates."}

tb.mod <-predict_ionize(df = tb.zero[[2]], 
                        Xt = Xt.pr, 
                        N = N.pr, 
                        t = t.rw, 
                        species = species.nm, 
...
tb.R <- stat_R(tb.mod, Xt.pr.l0, N.pr.l0, species.nm, ion1[2], ion2[2], file.nm , output = "complete")

ggplot(tb.mod, aes(x = t.rw, y = Xt.pr)) +
  geom_point(alpha = 0.1) +
  geom_line(aes(x = t.rw, y =  Xt.pr.model), color = "white") +
  facet_wrap(vars(species.nm), scales = "free")  +
  ylab("X (ct/sec)") +
  xlab("t")
...
```{r ionR4, echo=FALSE, fig.width=8, fig.height=4, fig.cap="An extraction of the $l0$ and $\\epsilon_t$ components, by appliciation of Eq. \\@ref(eq:detr), and thereby yielding corrected none-isotope ion coun ratios."}
ggplot(tb.R, aes(x = t.rw.12C14N, y = R_Xt.pr.l0)) +
  geom_point(alpha = 0.1) +
  facet_wrap(vars(R.nm), scales = "free") +
  ylab("R") +
  xlab("t")
