...
* Analysis: multitude of measurements $N_{(i)}$ (count cycles of length $n$)
* Study: $m$-series of analysis, constituting the different spots on the analytical substrate 
...
This function can also make correction on the ion counts associated with the machine setup, such as, artifacts induced by the ion detector type. These settings mostly affect the accuracy of the analysis. For more information on this topic see the vignette *IC-process*.
...
and yields an estimate for the sample correlation coefficient, where values can range between $-1$ and $+1$, and thereby recording a inverse or positive linear correlation between the variables, and no correlation if $r$ falls close to zero. The product of $r_{(z_j, z_k)}$, $s_j$, and $s_k$ is the same as the co-variance between of the two input variables, as such, the variable simplifies to $s_{jk}$. For this calculation the `base` function `cov()` was used, with the `method` argument set to `"pearson"` and `use` to `"everything"`.

Recasting [@eq:er.prop] for when $F(...)$ is $R$, and with the variables $\bar{X}^{b}$ (e.g. ^13^C) and $\bar{X}^{a}$ (e.g. ^12^C), yields the following equation:
...
The correlation coefficient ($r$) becomes zero, as the count statistics for both isotopes are independent. The predicted standard deviations for $N^{b}$ and $N^{a}$ can be approximated by the population mean, according to [@eq:std.pois], thereby transforming [@eq:std.pois.R1];
...
The predicted standard deviation ([@eq:std.pois.R5]) and standard error of the mean ([@eq:std.pois.R6]) can again be expressed as relative uncertainties in ‰, following the same transformation as for the descriptive statistics.         
