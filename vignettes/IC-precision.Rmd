...
tb_rw <- purrr::map2(
  ion1, 
  ion2, 
  ~zeroCt(tb_rw, .x, .y, file.nm, sample.nm, .N = N.rw, .warn = FALSE)
  ) %>% 
...
The ion counts obtained from the 2018-01-19-GLENDON dataset includes the species: `r sapply(unique(tb_rw$species.nm), ion_labeller, "latex")`, of which some are poly-atomic.
...
tb_Xt <- stat_Xt(tb_pr, sample.nm, file.nm, .stat = c("Ntot", "M", "S", "SeM"), 
                 .label = "latex")
```
...
tb_R <- stat_R(tb_pr, "13C", "12C", sample.nm, file.nm, .label = "latex", 
...
To calculate the external precision for ion ratios with the package `point` apply the functions `stat_Xt()` and `stat_R()` in conjunction on the same dataset (e.g. a raw ion count dataset containing replicate measurements on a reference material). Note, that the second call should calculate the statistics over the summary table of the first iteration.
...
tb_R_ext  <- stat_R(tb_Xt, "13C", "12C", sample.nm, .Xt = M_Xt.pr, 
                    .N = Ntot_Xt.pr, 
                    .stat =  c("M", "RS", "RSeM", "hat_RS", "hat_RSeM", "chi2"), 
                    .label = "latex")
```
...
tb_R <- stat_R(tb_pr, "13C", "12C", sample.nm,  file.nm, .zero = TRUE)

tb_R_ext <- stat_R(tb_Xt, "13C", "12C", sample.nm, .Xt = M_Xt.pr, 
                   .N = Ntot_Xt.pr, .zero = TRUE)

# The average of the internal relative predicted standard error of the mean 
# (per mille)
filter(tb_R, str_detect(sample.nm, "Belemnite")) %>%  
